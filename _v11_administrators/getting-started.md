...
* An **application** container running python and gunicorn.
...
Materia's server application is built on top of [django](https://www.djangoproject.com/), an open-source python web framework. It requires a MySQL database (MariaDB will work too). The front-end code is written in React and makes use of the [React Query](https://tanstack.com/query/v4/docs/framework/react/overview) library to improve query performance.
...
The application is relatively self-contained within the `python` image, though certain resources specific to your instance of Materia should be mounted from the host.

For a deep dive into configuring your docker compose file(s), visit [Setting Up Docker](setup-configure-docker.html).

### Configuring Environment Variables

Materia has a number of critical settings that need to be configured before your instance is up and running. Because we want to avoid direct changes to configuration files within the application container, server settings are best written to **environment variables** that are then written to the container by docker compose.
...
We generally recommend storing your environment variables in a `.env` file that's then associated with the `env_file:` directive in the `python` service.

The [.env_template file in the Materia repo](https://github.com/ucfopen/Materia/blob/master/docker/.env_template) can serve as a template for your own `.env` file. We go into even greater detail on server variable configuration at the [Setting Up Server Variables page](setup-configure-env-vars.html).

### Network Configuration

We provide a baseline NGINX configuration that should be suitable for general needs, but specific changes may be required for your specific network requirements. If a custom NGINX configuration is not explicitly volume mounted, the [production configuration](https://github.com/ucfopen/Materia/blob/master/docker/config/nginx/sites-enabled/site.prod.conf) will be used.
...
1. Direct authentication through an auth module. We do not provide an out-of-the-box solution for user management with this approach. In most circumstances, a custom auth module must be written to facilitate user authentication from an upstream login, such as SAML. While django has command-line and programmatic tools to create new users, this is not sufficient for user management at scale.
...
The Materia application image does not come with any widgets pre-installed; once your instance is running, you can install widgets through django management commands or from the widget admin panel. The admin panel is accessible to superusers.

More information about widget installation and management is available on the [Managing Widgets](managing-widgets.html) page.
